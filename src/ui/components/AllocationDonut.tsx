...
export function AllocationDonut({ holdings, quotes, fxRates, baseCurrency }: AllocationDonutProps) {
  ...
  return (
    <div className="mb-6 rounded border p-4">
        ...
        <div className="flex gap-1 text-xs">
          {GROUP_OPTIONS.map((g) => (
            <button
              ...
              className={`rounded px-2 py-1 ${groupBy === g ? 'bg-blue-600 text-white' : 'bg-gray-100'}`}
            >
              {g}
            </button>
          ))}
      ...
      {slices.length === 0 ? (
        <p className="text-sm text-gray-500">No priced holdings yet.</p>
      ) : (
        <ResponsiveContainer width="100%" height={250}>
          <PieChart>
            <Pie data={slices} dataKey="value" nameKey="label" innerRadius={60} outerRadius={90}>
              {slices.map((s, i) => (
